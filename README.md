...
* **Contact details are set.** The shop phone `(317) 417-6240`, email `justin@jl-customs.com`, and site `jl-customs.com` are wired into the home screen, the printed estimate/receipt, and the backend "Contact an Expert" email. If they change, update `user/` and `user-linux/` `home.html` + `estimate.html` (the `RECEIPT_CONTACT` constant) and `backend/server.js`.
